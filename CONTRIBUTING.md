...
SnapshotBus (revogado 2026-07-15) → Art. 11.4
...
§1º (2026-07-15) `CurrentState` assume, explicitamente, o papel de **último estado físico confirmado da planta** — não só um repositório interno, mas a fronteira de leitura pra tudo que está fora da Thread da planta (Thread do Adaptador OPC-UA, futuro Controlador). `EvaluationState` continua thread-local, sem exceção — nunca é lido de fora. Ver Art. 3.6.2 (tipo interno) e Art. 3.6.6 (o que isso implica pra `Sensor`).
...
**Art. 3.6.2.** ~~`current_state` é `Rc<RefCell<Vec<Cell<f64>>>>` — mesma forma de `evaluation_state` (Art. 8), mutado célula-a-célula por `commit()`, nunca substituído por inteiro.~~ **Nota (2026-07-15):** `current_state` virou `Arc<RwLock<CurrentState>>`, onde `CurrentState { generation: u64, values: Vec<f64> }` — deixou de ter a mesma forma de `evaluation_state` (que continua `Rc<RefCell<Vec<Cell<f64>>>>`, thread-local, Art. 8). Motivo: `current_state` precisa atravessar thread (Art. 1.3, §1º) — `Rc`/`Cell` não são `Sync`, não há como compartilhar com segurança. `commit()` (Art. 9.8) passou a tomar o `write()` lock uma única vez por chamada, escrevendo todos os valores e avançando `generation` na mesma seção crítica — nunca célula-a-célula. `StateRegistry::read_proxy(key) -> Option<ReadProxy>` resolve a chave uma única vez contra esse buffer — é o que `Sensor` (Art. 3.6.3, 3.6.6) usa, agora via `ReadProxy::get_versioned() -> (u64, f64)` (geração + valor, mesmo lock). `StateRegistry::read(key) -> Option<f64>` continua existindo, mas como leitura pontual de debug/inspeção avulsa, não o caminho usado por `Sensor`.
...
**Art. 3.6.6.** (2026-07-15) `Sensor` é a camada de medição sobre `CurrentState` (Art. 1.3, §1º) — nenhum consumidor externo (cliente OPC-UA, futuro Controlador, Art. 3.5.1 §2º) lê o estado bruto diretamente quando o sinal em questão é um sensor; todos passam por `Sensor::read()`, que aplica `SensorBehavior` (escala, ruído, histerese, falha, transformação 4-20mA etc. — propriedade do instrumento, não da planta) antes de expor.

§1º `Sensor` virou `Send + Sync` — compartilhável via `Arc<Sensor>` entre a Thread da planta, a Thread do Adaptador e um futuro Controlador, todos apontando pro mesmo instrumento, sem cópia. `read()` passou a ser `&self` (não `&mut self`): a mutação de `SensorBehavior` fica atrás de um `Mutex` interno.

§2º Idempotência: `read()` é idempotente dentro da mesma `generation` de `CurrentState` (Art. 3.6.2). `Sensor` guarda um cache `(generation, valor_já_processado)` — a primeira chamada depois de um `commit()` invoca `SensorBehavior::apply()` de verdade (avança RNG de `Noisy`, reavalia deadband de `Hysteresis`) e guarda o resultado; qualquer chamada seguinte, de qualquer consumidor, de qualquer thread, antes do próximo `commit()`, só devolve o valor cacheado. Garante que OPC-UA e Controlador, lendo o mesmo sensor no mesmo tick, vejam sempre a mesma medição — `SensorBehavior` nunca avança duas vezes pro mesmo instante confirmado.

§3º Consequência de escopo: isso vale só pra sinais que são `Sensor` — leitura de debug/inspeção avulsa via `StateRegistry::read(key)` (Art. 3.6.2) continua expondo o valor bruto de `CurrentState`, sem `SensorBehavior` nem cache; não é o caminho que OPC-UA/Controlador usam pra sinais de sensor.
...
**Art. 11.3.** `run(self) -> Result<(), String>` consome a `Simulation` por valor. Dentro de `std::thread::spawn`: cria `StateRegistry::shared()`, chama a `model_factory` guardada, `resolve()` (Art. 6.3), constrói cada `Sensor`/registra cada `CommandSink` na `IoImage` — só agora esses objetos existem de verdade. Loop: drena comandos pendentes → `model.evaluate()` + `registry.commit()` ~~→ publica cada sensor no `SnapshotBus` (Art. 11.4)~~ → dorme `tick_interval`. **Nota (2026-07-15):** não há mais passo de publicação — `commit()` já é o suficiente, `Sensor` lê `CurrentState` sob demanda (Art. 3.6.6). Se `set_adapter()` foi chamado, uma segunda thread sobe (Art. 11.5).
...
- **`CommandQueue`** (`Arc<Mutex<std::sync::mpsc::Sender<(String, f64)>>>`): quem está fora empurra `(nome, valor)`; a planta drena no início de cada tick. O `Mutex` existe porque um write callback do OPC-UA exige `Fn(...) + Send + Sync` e `Sender` sozinho não é `Sync`. Não sabe o que é OPC-UA, TEP ou `StateRegistry`.

**Nota (2026-07-15):** `SnapshotBus` foi eliminado. `CurrentState` (Art. 1.3 §1º, 3.6.2) assumiu, explicitamente, o papel de "último estado físico confirmado" — não precisa mais de publicação intermediária: `Sensor` (Art. 3.6.6) virou `Send + Sync` e é exportado, uma única vez, via `Arc<Sensor>` no handshake de boot (Art. 11.8); qualquer consumidor (adaptador, futuro Controlador) chama `sensor.read()` direto, sem canal nenhum no meio. `CommandQueue` não é afetado — motivo de existir é outro (escrita externa, não coerência de leitura) e continua como está.

§1º (2026-07-14, histórico — sobre o `SnapshotBus` já revogado acima) Achado em discussão sobre simplificar a ponte de leitura, depois verificado direto no código: a implementação original de `SnapshotBus::publish(name, value)` tomava o `write()` lock **uma vez por sensor**, dentro do `for name in &sensor_names` do loop de tick (`Simulation::spawn_plant_thread`). Como o lock era liberado entre um sensor e o próximo, um leitor concorrente (a "Thread do OPC-UA") podia — em teoria — ler o `HashMap` no meio dessa sequência e enxergar uma mistura de valores de ticks diferentes entre variáveis distintas: não havia, de fato, uma "foto" atômica do tick inteiro, ao contrário do que o nome `Snapshot`Bus sugere. Corrigido, na ocasião, trocando `publish()` por `publish_all(...)` (lock único por tick) — mudança inteira superada pela eliminação do `SnapshotBus` no dia seguinte; mantido aqui só como registro histórico do raciocínio que levou até a eliminação.

**Art. 11.5.** ~~`opcua_adapter::serve(sensor_names, actuator_names, snapshot: SnapshotBus, commands: CommandQueue, endpoint)`~~ **Nota (2026-07-15):** assinatura atual é `opcua_adapter::serve(sensors: HashMap<String, Arc<Sensor>>, actuator_names, commands: CommandQueue, endpoint)` — não importa `Simulation`/`IoImage`/`StateRegistry` — só o catálogo de sensores (Art. 3.6.6, 11.8) e `CommandQueue` (Art. 11.4). O loop de push chama `sensor.read()` direto em cada `Arc<Sensor>` a cada tick (500ms) — não existe mais `snapshot.read(name)`. O `add_write_callback` de cada atuador só chama `commands.write(name, value)`. Como nada aqui é `!Send`, o loop de push roda em `tokio::spawn` comum. `run()` cria o runtime tokio só dentro da "Thread do OPC-UA" — o resto do processo nunca vê tokio.

§1º (2026-07-15) O runtime criado em `spawn_adapter_thread` é `tokio::runtime::Builder::new_current_thread()`, não `Runtime::new()`/`multi_thread`. Motivo: `async-opcua-server`/`async-opcua-core` dependem de `tokio` direto (`features = ["full"]`, confirmado no `Cargo.toml` publicado da lib) — não existe abstração de runtime async trocável; e o adaptador em si é puramente I/O-bound (um `TcpListener`, poucas conexões, uma tarefa de push a cada 500ms via `tokio::spawn`/`tokio::time::interval`), sem trabalho paralelo real que justifique o pool de worker threads (um por núcleo lógico da máquina) que `multi_thread` cria por padrão. A troca não muda nada em `opcua_adapter::serve` nem na lib — só o `Builder` usado em `spawn_adapter_thread`; validado subindo `tep-plant` de verdade e conectando um cliente OPC-UA (sessão, `Browse`, os 4 nodes esperados), comparado lado a lado contra a baseline `multi_thread` pra isolar um bug pré-existente (leitura de `Value` via `ReadAttribute` retorna `BadNodeIdUnknown` em ambos) como não-regressão desta troca.
