...
BootCatalogs          → Art. 12.4
Valve/Agitator        → Art. 2.2, 2.6, 12.8
...
§1 (open item — resolved, see Art. 12.3 §1, 2026-07-29) `add_actuator(name, sink: impl CommandSink + Send + 'static)` works for sinks that only capture external `Send` data (e.g. a channel, an `Arc<AtomicXxx>`). It doesn't work for something like `move |v| valve.set_command(v)`, because `Valve` only exists *after* the `model_factory` runs inside the "plant thread" — at the moment `add_actuator()` is called (before `run()`), that `Valve` hasn't even been built yet. Today this doesn't block anything because no real actuator (`Valve`/`Agitator`) is wired into `TennesseeEastmanModel` yet (Art. 2.6, §1) — but once one exists, it will need a "key resolved later" mechanism, symmetric to what `add_sensor` already has via `key: &str`, instead of accepting a ready-made closure.

**Art. 11.3.** `run(self) -> Result<(), String>` consumes `Simulation` by value. Inside `std::thread::spawn`: it creates `StateRegistry::shared()`, calls the stored `model_factory`, `resolve()`s (Art. 6.3), ~~builds each `Sensor`/registers each `CommandSink` in the `IoImage`~~ builds both catalogs (`build_catalogs`, Art. 12.4), 2026-07-29 — only now do these objects really exist. Loop: ~~drain pending commands →~~ `model.evaluate()` + `registry.commit()` ~~→ publish each sensor to the `SnapshotBus` (Art. 11.4)~~ → sleep `tick_interval`. **Note (2026-07-15):** there is no longer a publish step — `commit()` is already enough, `Sensor` reads `CurrentState` on demand (Art. 3.6.6). **Note (2026-07-29):** there is no longer a per-tick drain step either — actuator commands are consumed via `Actuator::take()` by whichever model holds its own `Arc<Actuator>` clone, entirely outside `Simulation`'s loop (Art. 12.3). If `set_adapter()` was called, a second thread comes up (Art. 11.5, altered by Art. 12.5).
...
**Art. 12.1.** (2026-07-29) `IoImage`, `CommandSink`, and `CommandQueue` were eliminated. `Actuator` (`monjolo/src/actuator/model.rs`) now mirrors `Sensor`'s design (Art. 3.6.6) on the write side: `Send + Sync`, holding its state behind an internal `Mutex`, exported exactly once via `Arc<Actuator>` in the same boot handshake that already exported `Arc<Sensor>` (Art. 11.8). No bridge object exists for either direction anymore — reads and writes are now symmetric.

**Art. 12.2.** `Actuator` is a mailbox, not a queue: `write(&self, value: f64)` overwrites any not-yet-consumed pending value (last-write-wins, no history — a control command only ever needs to be the latest one); `take(&self) -> Option<f64>` consumes it, returning `None` if nobody has written since the last `take()`. Both methods are `&self`, not `&mut self` — mutation happens behind the internal `Mutex`, same reasoning as `Sensor::read()` (Art. 3.6.6 §1): `Actuator` is meant to be shared via `Arc` between threads, with no exclusive owner.

**Art. 12.3.** `Actuator` does not know about `DynamicModel`, `Valve`, or `Agitator` (`monjolo/src/actuator/dynamic.rs` — a different, unrelated concept: physical first-order-lag dynamics participating in `evaluate()`/RK4, not a write mailbox). `Simulation::add_actuator(name: &str, actuator: Arc<Actuator>)` only catalogs the `Arc` for export to the adapter thread — it never calls `write()`/`take()` on it itself. Consuming a written command (e.g. calling `Valve::set_command()` with the value from `take()`) is entirely the responsibility of whichever model holds its own clone of the `Arc<Actuator>`, done inside that model's own `evaluate()` — `Simulation`'s tick loop no longer drains anything for actuators (Art. 11.3, altered).

§1 (2026-07-29) This resolves the pending item from Art. 11.2 §1: because `add_actuator` now receives an already-constructed `Arc<Actuator>` instead of a closure capturing a not-yet-built component, the ordering problem described there (`Valve` not existing yet when `add_actuator()` is called, before `run()`) never arises — `Actuator` needs nothing from the model to exist; the model reaches for it later, on its own schedule. ~~No actual `Valve`/`Agitator` is wired into `TennesseeEastmanModel` yet (Art. 2.6, §1) — this article settles the mechanism, not the wiring.~~ **Note (2026-07-30):** wired now — see Art. 12.8.

**Art. 12.4.** `Simulation`'s boot handshake (Art. 11.8) exports both catalogs at once via a private `BootCatalogs { sensors: HashMap<String, Arc<Sensor>>, actuators: HashMap<String, Arc<Actuator>> }` — plain data, no dispatch/lookup methods on it (that would be reinventing `IoImage`, Art. 10.1). Built by a private `Simulation::build_catalogs()` helper: merges `add_sensor()`'s external specs with the model's own declared `sensors()` (same merge `IoImage` used to do, Art. 10.3, superseded), and collects the `(name, Arc<Actuator>)` pairs from `add_actuator()` as-is. Extracted specifically so catalog assembly is unit-testable without spawning a real thread.

**Art. 12.5.** Current `opcua_adapter::serve()` signature: `serve(sensors: HashMap<String, Arc<Sensor>>, actuators: HashMap<String, Arc<Actuator>>, endpoint: &str) -> Result<(), String>` — supersedes Art. 11.5. No `actuator_names: Vec<String>`, no `commands: CommandQueue` — the actuator catalog carries everything the adapter needs, symmetric to the sensor catalog it already received.

**Art. 12.6.** The OPC-UA write callback (`SimpleNodeManager::add_write_callback`, `Fn(...) + Send + Sync + 'static`) captures its own bound `Arc<Actuator>` per node directly — one per `for (name, actuator) in actuators` loop iteration — and calls `actuator.write(value)`. No channel, no name-based lookup at write time (supersedes Art. 10.6.2).

**Art. 12.7.** There is no `sensor_names()`/`actuator_names()` method anywhere anymore (supersedes Art. 10.6.4) — the adapter receives two already-typed `HashMap`s (Art. 12.4) and iterates each one directly: which map a name came from already tells it whether that node is read-only or writable.

**Art. 12.8.** (2026-07-30) `DynamicModel` gained `actuators(&self) -> Vec<(String, Arc<Actuator>)>` (`monjolo/src/dynamic_model.rs`), mirroring `sensors()` (Art. 11.8) on the write side — empty default, only whoever orchestrates overrides it. `Simulation::set_model()` captures it at the same moment as `sensors()`/`state_keys()`; `Simulation::build_catalogs()` (Art. 12.4) merges these model-declared actuators with whatever was registered externally via `add_actuator()`, the same way it already merges the two sensor sources.

`Valve`/`Agitator` (`monjolo/src/actuator/dynamic.rs`) each gained a field holding their own `Arc<Actuator>` (constructed by whoever builds them), consumed via `incoming.take()` at the start of `evaluate()`, before computing the derivative — a written command overwrites `command` (now `Cell<f64>`, not a plain field, since `evaluate(&self)` can't take `&mut self`) if one arrived since the last tick, otherwise the last known `command` is reused unchanged.

§1 (2026-07-30) `TennesseeEastmanModel::new()` (Art. 2.6) now builds this for real: one `Valve` (name `"cooling_water"`, τ from `VTAU(10)` in `teprob.f`, 5s) and the plant's one `Agitator` (τ from `VTAU(12)`, also 5s), each constructed with its own `Arc<Actuator>` — one clone `add_dynamic`'d into the component, the other clone handed to `actuators()`'s returned list under `"TEP/Reactor/CoolingWaterValve/Command"`/`"TEP/Reactor/Agitator/Command"`. Each is paired with a companion `Sensor` (`"TEP/Reactor/CoolingWaterValve/Position"` → `valve.cooling_water.position`, `"TEP/Reactor/Agitator/Speed"` → `agitator.speed`) so a client that writes a command has a way to confirm it was applied — position moves via the first-order lag (Art. 12.2's caput), not instantly. Not all 12 real TEP valves (XMV 1-11) are wired, only this one plus the agitator (XMV-12) — same "representative subset" scope already used for sensors (Art. 2.6, §1, only 4 of 41 XMEAS exposed).
