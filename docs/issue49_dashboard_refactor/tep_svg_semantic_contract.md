...
| `stream-07-reactor-condenser`   | segmento   | Stream 7   | —     | —   | reator → condensador                       |
| `stream-07-condenser-separator` | segmento   | Stream 7   | —     | —   | condensador → separador                    |
| `stream-08-up`                  | segmento   | Stream 8   | 5     | 5   | reciclo — trecho superior                  |
| `stream-08-down`                | segmento   | Stream 8   | 5     | 5   | reciclo — trecho inferior                  |
| `stream-08.1-bypass-up`         | segmento   | Stream 8.1 | —     | 5   | bypass do reciclo — trecho superior        |
| `stream-08.1-bypass-down`       | segmento   | Stream 8.1 | —     | 5   | bypass do reciclo — trecho inferior        |
| `stream-09-purge-up`            | segmento   | Stream 9   | 10    | 6   | purga — trecho superior; ponto XMEAS 29–36 |
| `stream-09-purge-down`          | segmento   | Stream 9   | —     | —   | purga — trecho inferior                    |
| `stream-10-separator-out`       | segmento   | Stream 10  | 14    | 7   | saída do separador (bottoms) → stripper    |
| `stream-10-up`                  | segmento   | Stream 10  | —     | —   | trecho vertical                            |
| `stream-10-down`                | segmento   | Stream 10  | —     | —   | trecho descendente                         |
| `stream-11-stripper-out`        | **âncora** | Stream 11  | 17    | 8   | saída do stripper; ponto XMEAS 37–41       |
| `stream-11-up`                  | segmento   | Stream 11  | —     | —   | trecho vertical                            |
| `stream-11-product-out`         | segmento   | Stream 11  | —     | —   | saída final do produto                     |

> **Renomeações upstream/downstream (v3):** `stream-10-in` → `stream-10-separator-out`; `stream-11-product` → `stream-11-stripper-out`; `stream-11-down` → `stream-11-product-out`; `stream-08-bypass-*` → `stream-08.1-bypass-*`. Stream 7 agora tem dois segmentos explícitos.

### 3.3 Correntes de utilidade — água de resfriamento e vapor

| data-cell-id                    | tipo        | XMV | XMEAS | observação                                       |
| ------------------------------- | ----------- | --- | ----- | ------------------------------------------------ |
| `stream-12-cws-reactor-in`      | CWS in      | 10  | —     | água fria entrando no reator                     |
| `stream-12-cws-reactor-out`     | CWS out     | —   | 21    | água quente saindo do reator                     |
| `stream-12-cws-reactor-up`      | segmento    | —   | —     | trecho vertical — circuito CWS reator            |
| `stream-13-cws-condenser-in`    | CWS in      | 11  | —     | água fria entrando no condensador                |
| `stream-13-cws-condenser-out`   | CWS out     | —   | 22    | água quente saindo do condensador                |
| `stream-13-cws-condenser-up`    | segmento    | —   | —     | trecho vertical — circuito CWS condensador       |
| `stream-14-up`                  | viz interna | —   | —     | linha de utilidade térmica do stripper (subida)  |
| `stream-14-down`                | viz interna | —   | —     | linha de utilidade térmica do stripper (descida) |
| `stream-14-out`                 | viz interna | —   | —     | saída da linha de utilidade do stripper          |
| `stream-15-stripper-boiler-in`  | processo    | —   | —     | líquido do stripper entrando no reboiler         |
| `stream-15-stripper-boiler-out` | processo    | —   | —     | líquido saindo do reboiler para o stripper       |

> **IDs 12 e 13** são streams oficiais TEP (CWS). **IDs 14 e 15** são IDs internos de visualização — sem número de stream TEP, sem binding XMEAS/XMV. O vapor de aquecimento (XMV 9 / XMEAS 19) é controlado via `actuator-xmv-09` e `sensor-xmeas-19`; seus segmentos visuais estão em `stream-14-*`.
...
- para múltiplos segmentos: `querySelectorAll('[data-cell-id^="stream-08-"]')` (stream 8 principal); bypass usa `[data-cell-id^="stream-08.1-"]`.
...
function updateStream11(snap) {
  ...
  // Âncora primária
  const anchor = document.querySelector('[data-cell-id="stream-11-stripper-out"]');
