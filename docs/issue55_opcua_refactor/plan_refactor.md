...
### 9.9. Implementado — `simulation-framework/src/method/{integrator.rs,rk4.rs}` + `Simulation::run_model()`

As seções 9.1-9.8 saíram do papel. Detalhes que o texto acima não fixava e que a implementação precisou decidir:

- **De onde vem a lista de nomes da 9.3**: mesmo padrão de `DynamicModel::sensors()` (seção 11.8) — um novo método default-vazio, `DynamicModel::state_keys(&self) -> Vec<String>`, que cada modelo sobrescreve declarando só a chave-base (ex.: `"valve.feed_a.position"`); a chave da derivada é sempre `"<key>.derivative"` por convenção (já era a convenção de `Valve`/`Agitator`, seção 3 dos atuadores). `Simulation::set_model()` captura `model.state_keys()` no mesmo momento em que já captura `model.sensors()` (tipo ainda concreto). Dentro de `run_model()`, cada par `(key, key.derivative)` vira dois `need`s em `subscribe(&[], ...)` — reaproveita o mecanismo de resolução existente, nenhuma API nova no `StateRegistry`.
- **`tick_interval` (ritmo de parede) ≠ `dt` (passo físico simulado)**: usar o intervalo de `sleep()` como `dt` do RK4 estaria errado (segundos de parede não são horas de processo). `Simulation` ganhou um campo `dt_hours` (setter `set_dt_hours`), independente de `tick_interval` (setter `set_tick_interval`).
- **Passo extra não previsto no texto original**: depois de `Integrator::step()` devolver o estado combinado (`y_new`), o valor em `EvaluationState` ainda reflete o último sub-passo `k4` (avaliado em `y + dt·k3`, um ponto hipotético) — não `y_new`. `run_model()` faz uma escrita + `evaluate()` extra depois do `step()`, só com o estado final de verdade, antes de commitar — senão os valores algébricos citados na 9.8 (ex.: `reactor.temperature`) ficariam consistentes com um sub-passo intermediário, não com o estado que de fato foi commitado.
- **Estado real hoje (2026-07)**: `Valve`/`Agitator` são os únicos `DynamicModel`s com par estado+derivada de verdade — mas nenhum dos dois está `add_dynamic`'d dentro de `TennesseeEastmanModel` ainda (pendência separada, seção 11.7). `Reactor`/`Separator`/`Stripper`/`Compressor` não declaram `state_keys()` (a derivada do `own_state` deles é responsabilidade de `Flows`, ainda não implementado — ver comentário "decisão de modelagem" no fim do `evaluate()` de cada um). Na prática, `TennesseeEastmanModel::state_keys()` devolve vazio hoje, e o binário `tep-plant` roda no mesmo modo `state_proxies.is_empty()` de antes do Integrator existir — a máquina está pronta e testada (`numerical_method::rk4::tests`, `simulation::tests::run_integrates_declared_state_keys_via_rk4`), mas só passa a mexer no estado real da planta quando Valve/Agitator forem wireados e/ou `Flows` existir.
- **Atualização (11.10): `RK4` deixou de ser hardcoded.** `Simulation::set_numerical_method(NumericalMethod)` (seção 9.9 original) agora é consumido de verdade dentro de `spawn_plant_thread` via `NumericalMethod::integrator() -> Box<dyn Integrator>` — antes disso, o campo existia mas `run_model()` ignorava e usava `RK4` fixo. `tep_plant.rs` chama `simulation.set_numerical_method(NumericalMethod::RK4)` explicitamente em vez de depender só do default.
...
### 11.10. `Simulation` virou lifecycle manager/supervisor — `run_model()` → `run()`, `start_opcua_server()` → `set_adapter()`

Motivação: `run_model()` (11.3) misturava, numa função só, "o que construir" (StateRegistry/modelo/IoImage), "em que ordem subir as threads" e "qual `.join()` esperar" — e assumia implicitamente que sempre existiria uma planta (`set_model()` obrigatório) com o adapter como acessório opcional. Analogia discutida: por que `Simulation` não pode embutir Tokio/OPC-UA do mesmo jeito que Spring embute Tomcat? Resposta: pode — a única regra concreta que restringe isso é `Send` (`Rc<RefCell<StateRegistry>>` não atravessa thread, por decisão de performance do caminho quente, não por limitação de Rust em geral); nada impede `Simulation` de ser exatamente esse tipo de host embutido.

**`AdapterConfig`** (`simulation-framework/src/adapter/mod.rs`) — mesmo padrão de `NumericalMethod` (seção 9.9): enum fechado, hoje só `OpcUa { endpoint: String }`, variante existindo só com a feature `opcua` (sem a feature, o enum fica sem nenhuma variante construível, mas continua compilando). `Simulation::set_adapter(config: AdapterConfig)` substitui `start_opcua_server(endpoint: &str)`.

**`run()`** (`Simulation::run_model` renomeado) agora orquestra 0/1/2 serviços dependendo do que foi configurado: sobe a "Thread da planta" só se `set_model()` foi chamado, sobe a "Thread do adapter" só se `set_adapter()` foi chamado, e devolve `Err` sem subir nada se nenhum dos dois foi configurado — antes, `set_model()` era obrigatório incondicionalmente.

**Supervisor (`ServiceEvent`/`ServiceKind`, privados em `simulation.rs`)** — resolve a lacuna identificada: antes, se dois serviços estavam configurados, `run_model()` só dava `.join()` numa das duas threads (a do adapter); a outra ficava órfã, sem supervisão — um pânico nela não era percebido por ninguém. Agora, cada thread interna roda o corpo inteiro dentro de `std::panic::catch_unwind` e manda exatamente um `ServiceEvent` (`Stopped`/`Failed`/`Panicked`) por um canal de lifecycle como último passo antes de retornar — `Failed` é erro fatal sem pânico (ex.: `serve()` do OPC-UA devolvendo `Err`, que antes só dava `eprintln!` e seguia; agora propaga de verdade), `Panicked` carrega a mensagem extraída do payload do pânico. `run()` bloqueia em `events_rx.recv()`: o primeiro evento de qualquer serviço configurado já é motivo pra retornar — testado com um modelo que entra em pânico de propósito depois de alguns ticks saudáveis (`run_returns_err_instead_of_hanging_when_plant_panics`), provando que `run()` devolve `Err` em vez de travar pra sempre.

**Limitação conhecida, deixada explícita de propósito:** não existe cancelamento cooperativo — nem a plant thread nem a thread do adapter checam um sinal de parada, então quando uma morre a outra (se houver) não é avisada; `run()` só para de esperar por ela (nunca chama `.join()` nela, bloquearia pra sempre enquanto ela seguir saudável). Cabe a quem chamou `run()` decidir encerrar o processo — o caso comum de um binário, onde isso mata a thread órfã junto. Shutdown gracioso de verdade (sinal de parada cooperativo pras duas threads) fica como próximo passo, não fazia parte do pedido desta rodada.

### 11.11. Pendência da seção 11.9 superada: `Separator`/`Stripper`/`Compressor` também consomem `Snapshot`

`Separator::new(registry, initial: &Snapshot)` já tinha sido implementado no mesmo commit que criou `Reactor::new` (seguia o texto original desta seção por engano — a pendência real era só `Stripper`/`Compressor`). `Stripper::new` e `Compressor::new` ganharam o segundo parâmetro `initial: &Snapshot`, seguindo exatamente o padrão de `Reactor`/`Separator`: cada um semeia seu `own_state` a partir das chaves que já existiam no `te_exp3_snapshot.toml` (`state.stripper_liquid.{A..H}`/`state.stripper.energy` e `state.compressor_vapor.{A..H}`/`state.compressor.energy`), chave ausente vira `0.0`. `TennesseeEastmanModel::new` repassa `initial` para os quatro subsistemas. Testes `new_seeds_own_state_with_initial_condition`/`new_defaults_missing_keys_to_zero` espelhados em `stripper.rs`/`compressor.rs`. Suíte completa (`cargo test`) e o teste de fiação (`wires_and_evaluates_without_panicking`) passam sem alteração de comportamento numérico.

## 12. Pendência central, hoje pouco visível: `Flows`/`Heat`/`Measurements` são stubs — sem eles, o núcleo do TEP não integra estado nenhum

Esta pendência já estava anotada de passagem no último bullet da seção 9.9 ("Estado real hoje"), mas enterrada como detalhe de implementação do Integrator, longe de onde qualquer um leria sobre `Flows`/`Heat`/`Measurements` (seções 3-4) ou da issue #58. Ganha seção própria porque é, na prática, o item que bloqueia todo o resto ficar significativo — atuador wireado (seção 11.7), controlador (seção 10.5) e validação IEC 62264 (issue #55) não produzem efeito real sobre a planta enquanto isto não existir.

### 12.1. O que existe hoje: três structs vazios, `evaluate()` é `todo!()`

`tep-plant/src/subsystems/{flows,heat,measurements}.rs` — cada um é um struct sem campos (`pub struct Flows;` etc.), `impl DynamicModel` só com `name()` e um `evaluate(&self) { todo!("...") }` que entra em pânico se chamado. Nenhum dos três está `add_dynamic`'d em `TennesseeEastmanModel::new()` (`model.rs`) — não são só "não implementados", são código morto: nunca instanciados em lugar nenhum, nem no binário `tep-plant`.

### 12.2. Consequência prática: `own_state` de Reactor/Separator/Stripper/Compressor nunca avança no tempo

Quem escreveria a derivada real (`yp`) do `own_state` desses quatro subsistemas é `Flows` — é o que o comentário "[DECISÃO DE MODELAGEM]" no fim do `evaluate()` de cada um já diz. Como `Flows` não existe, nenhum dos quatro oferece a chave `".derivative"` companheira do seu `own_state`, e `TennesseeEastmanModel` não sobrescreve `state_keys()` (fica no default vazio do trait, seção 9.9). Resultado: `Simulation`/RK4 não têm o que integrar para o núcleo químico — `own_state` fica congelado exatamente no valor semeado pelo `Snapshot` (seção 11.9/11.11), e `evaluate()` só recalcula grandezas termodinâmicas derivadas (temperatura, pressão, composição) em cima desse estado parado. A máquina de integração em si funciona e está testada (`Valve`/`Agitator` já a usam corretamente, `numerical_method::rk4::tests`, `simulation::tests::run_integrates_declared_state_keys_via_rk4`) — só não tem o que fazer aqui ainda.

### 12.3. Onde está a física original

`tep-plant/docs/_deprecated_2.rs` — comentado, não compilado, mas com a lógica original completa: `TepFlows` (Blocks 19-31 do `teprob.f`: composição/peso molecular/entalpia de corrente, fluxos por componente), `TepHeat` (Blocks 32-34: trocas térmicas reator/separador/stripper) e `TepMeasurements` (Block 35-36: preenche `xmeas[0..22]`, detecta shutdown). Portar exige decidir os `needs` de cada um sobre o `StateRegistry` atual (`Flows` precisa dos quatro subsistemas químicos + posições de válvula + flags de distúrbio ao mesmo tempo — bem mais numeroso que qualquer `need` existente hoje) e reescrever a lógica em cima de `Proxy`/`ReadProxy`, não `&[f64]` posicional.

### 12.4. Ordem de dependência revisada

`Flows` primeiro (ninguém mais depende dele, ele depende dos 4 químicos) → `Heat` (depende de Reactor+Stripper+Flows) → `Measurements` (depende de todos + Flows + Heat) → só então wireara atuador (seção 11.7) faz sentido de verdade, porque sem `Flows` calculando o efeito de vazão/posição de válvula sobre o estado, mover uma válvula não muda nada na planta.
